...
void FirstState::render() {
  ...
  drawBackground(window_, b2Vec2(1400, 900), pos);
  if (pos.y < -460) {
  ...
  // TODO(Goginet): сделать отображение информации о ракете
  // drawInfo(window);

  rocket_->render(window_, scale_);

  ground_->render(window_, 30, pos);
}
...
void FirstState::update(unsigned int deltaTime) {
  ...
  // TODO(Goginet): переделать когда планета станет круглой
  float distance = rocket_->getPlanetDistance(scale_);
  world_->SetGravity(b2Vec2(0, -computeGravity(distance)));
